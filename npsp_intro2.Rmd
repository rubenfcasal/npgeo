...
We can plot the trend estimates with function `simage()` (or `spersp()`). Nevertheless, in this case the estimates should be previously masked...
...
```{r }
bin <- binning(x, y, nbin = c(120, 120), window = border, warn = TRUE)
...
The modified cross-validation criteria (MCV), proposed in (Chu and Marron, 1991) for
time series, can be adapted for spatial data, by ignoring
...
tmp <- mask(bin, window = border) # , set.NA=FALSE, warn = TRUE)
bin <- binning(x, y, window = border) # , set.NA=FALSE, warn = TRUE)
lp0.h <- h.cv(bin)$h
...
with functions `np.svar()` and `np.svar.corr()`. The generic
...
# is.na(krig.grid2$kpred) <- !mask.sp
# is.na(krig.grid2$ksd) <- !mask.sp
...
the Xunta de Galicia (Grupos de Referencia Competitiva ED431C-2016-015
and Centro Singular de Investigación de Galicia ED431G/01), all of them
